...
function readManifest(compilation, manifest, opts) {
	...
	if(exists(manifest)) {
		...
		if(opts.create) {
			compilation.warnings.push(new Error('Localization resource manifest not found. Generating ' + manifest
					+ ' to prevent future iLib errors.'));
			var root = path.dirname(manifest);
			if(!exists(root)) {
				fs.mkdirSync(root);
			}
...
function handleBundles(compilation, manifests, opts, callback) {
	if(manifests.length === 0) {
		...
		try {
			...
			compilation.errors.push(new Error('Error: Unable to read localization manifest at '
					+ manifest));
			handleBundles(compilation, manifests, opts, callback);
		}
	}
	...
	if(typeof this.options.ilib === 'undefined') {
		try {
			...
			if(pkgName === '@enact/i18n') {
				...
				if(exists('node_modules/@enact/i18n/ilib')) {
					this.options.ilib = 'node_modules/@enact/i18n/ilib';
				}
			}
		} catch(e) {
