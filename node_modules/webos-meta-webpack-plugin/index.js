...
var sysAssetsPath = 'sys-assets';
var variableSysPaths = null;
...
function handleSysAssetPath(context, appinfo) {
	...
	var sys = path.join(context, sysAssetsPath);
	if(!variableSysPaths && exists(sys)) {
		var list = fs.readdirSync(sys);
...
function detectSysAssets(name) {
	...
	var result = [];
	var trueName = name.substring(1);
	for(var i=0; i<variableSysPaths.length; i++) {
		var abs = path.resolve(path.join(variableSysPaths[i], trueName));
		if(exists(abs)) {
			result.push(abs);
		}
	}
...
function addMetaAssets(metaDir, outDir, appinfo, compilation) {
	...
	for(var i=0; i<props.length; i++) {
		var p = props[i];
		if(appinfo[p]) {
			var assets = appinfo[p].charAt(0)==='$' ? detectSysAssets(appinfo[p]) : [path.resolve(path.join(metaDir, appinfo[p]))];
...
WebOSMetaPlugin.prototype.apply = function(compiler) {
	...
	compiler.plugin('emit', function(compilation, callback) {
		...
		var locFile, locRel, locMeta, locCode;
