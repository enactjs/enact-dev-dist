...
function isUsingPrerenderPlugin(config) {
	if(config && config.plugins) {
		for(var i=0; i<config.plugins.length; i++) {
			if(config.plugins[i] && config.plugins[i].constructor &&
					config.plugins[i].constructor.name==='PrerenderPlugin') {
				return true;
			}
		}
	}
...
WebOSMetaPlugin.prototype.apply = function(compiler) {
	...
	compiler.plugin('emit', function(compilation, callback) {
		...
		if(meta) {
			if(meta.obj && isUsingPrerenderPlugin(compilation.options)) {
				meta.obj.usePrerendering = true;
			}
