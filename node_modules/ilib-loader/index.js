...
function readManifest(manifest, loader) {
	...
	if(exists(manifest)) {
		...
		var mDir = path.dirname(manifest);
		files = glob.sync('./**/*.json', {nodir:true, cwd:mDir});
		for(var i=0; i<files.length; i++) {
			files[i] = files[i].replace(/^\.\//, '');
		}
		var data = JSON.stringify({files:files}, null, '\t');
		if(!loader.query || loader.query.indexOf('noSave')===-1) {
			loader.emitWarning('App-level localization resource manifest not found. Generating ilibmanifest.json to prevent future iLib errors.');
			if(!exists(mDir)) {
				fs.mkdirSync(mDir);
			}
